...
<head>
    ...
    <title>Spreadsheet Converter | NexusTools</title>
    ...
    <style>
     ...
     /* --- Footer --- */
        ...
        @media (max-width: 768px) {
            ...
            .converter-container { margin-top: -2rem; padding: 0 1rem; }
            .converter-card { padding: 2rem 1.5rem; }
            
            .upload-zone { padding: 3rem 1rem; }
            .upload-icon { font-size: 3rem; }
            .upload-text { font-size: 1.25rem; }
        ...
        @media (max-width: 480px) {
            ...
            #success-ui h3 { font-size: 1.5rem !important; }
        }
    </style>
</head>
<body>
    ...
    <!-- Hero Section -->
    <section class="hero">
        <h1>Convert Excel and CSV Instantly</h1>
        <p>Seamlessly transform your spreadsheets between highly-compatible CSV and Microsoft Excel (.xlsx) formats.</p>
    </section>
    ...
    <div class="converter-container">
        <div class="converter-card">
            ...
            <div class="mode-switch">
                <div class="mode-option active" data-mode="excel-to-csv" onclick="setMode('excel-to-csv')">
                    <i class="fa-solid fa-file-excel" style="margin-right: 8px;"></i> Excel to CSV
                </div>
                <div class="mode-option" data-mode="csv-to-excel" onclick="setMode('csv-to-excel')">
                    <i class="fa-solid fa-file-csv" style="margin-right: 8px;"></i> CSV to Excel
                </div>
            </div>
            ...
            <div id="upload-ui">
                <div class="upload-zone" id="drop-zone">
                    ...
                    <h3 class="upload-text" id="upload-text">Choose Excel file</h3>
                    <p class="upload-hint" id="upload-hint">or drop XLSX here</p>
            ...
            <div class="status-container" id="processing-ui">
                <div class="file-info">
                    <i class="fa-solid fa-file-excel" id="processing-icon"></i>
                    <span id="filename-display">document.xlsx</span>
                </div>
                
                <h3 id="progress-title" style="margin-bottom: 0.5rem;">Uploading Document</h3>
                ...
                <button id="cancel-btn" class="btn btn-outline error" style="margin-top: 1rem; padding: 8px 24px; font-size: 0.95rem;">
            ...
            <div class="status-container" id="success-ui">
                ...
                <h3 style="margin-bottom: 2rem; font-size: 1.8rem;">Your file is ready!</h3>
...
<section class="features">
    <h2 class="section-title">Why choose Spreadsheet Converter?</h2>
    <div class="features-grid">
        <div class="feature-card">
            <div class="feature-icon"><i class="fa-solid fa-bolt"></i></div>
            <h3>Fast & Easy</h3>
            <p>Simply drag and drop your Excel or CSV file and download your converted spreadsheet in seconds. No registration required.</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon"><i class="fa-solid fa-shield-halved"></i></div>
            <h3>100% Secure</h3>
            <p>Your files are processed on our secure servers and automatically deleted after 1 hour to ensure your data privacy.</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon"><i class="fa-solid fa-table"></i></div>
            <h3>Perfect Formatting</h3>
            <p>Convert between Excel and CSV while preserving all your data, formulas, and cell formatting with precision.</p>
        </div>
    </div>
</section>
    ...
    <script>
        ...
        function setMode(mode) {
            ...
            if (mode === 'excel-to-csv') {
                uploadText.textContent = 'Choose Excel file';
                uploadHint.textContent = 'or drop XLSX here';
                fileInput.accept = '.xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
                processingIcon.className = 'fa-solid fa-file-excel';
            } else {
                uploadText.textContent = 'Choose CSV file';
                uploadHint.textContent = 'or drop CSV here';
                fileInput.accept = '.csv,text/csv';
                processingIcon.className = 'fa-solid fa-file-csv';
            }
            resetUI();
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function handleFiles(files) {
                ...
                const file = files[0];
                const expectedExt = currentMode === 'excel-to-csv' ? '.xlsx' : '.csv';

                if (!file.name.toLowerCase().endsWith(expectedExt)) {
                    showError('Please upload a valid ' + expectedExt.toUpperCase() + ' file.');
                    return;
                }
            ...
            function uploadFile(formData) {
                // Generate a random ID to track progress safely
                const jobId = (currentMode === 'excel-to-csv' ? 'e2c_' : 'c2e_') + Math.random().toString(36).substr(2, 9);
                ...
                xhr.upload.onprogress = function(e) {
                    if (e.lengthComputable) {
                        ...
                        if (percentComplete >= 100) {
                            progressTitle.textContent = "Converting Data";
                            progressDesc.textContent = "Processing layout and formatting... Phase 2 of 2";
                            progressFill.style.width = '0%';
                            progressFill.style.background = '#059669'; 
                            progressPercentage.style.color = '#059669';
                            progressPercentage.textContent = '0%';
                            
                            // 2. Start polling for python conversion progress
                            startPolling(jobId);
                        }
                    }
                };
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try {
                            ...
                            if (data.success) {
                                ...
                                showError(data.error || 'Conversion failed. ' + (data.details || ''));
                                resetUI();
                            }
                        } catch(e) {
